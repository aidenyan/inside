...
setCheckRoleListInit=function(roleIdList){
   if(roleIdList==null ||roleIdList==undefined ||typeof (roleIdList)=="undefined"){
       return;
   }
...
saveAccountInfo = function (index) {
    var url = "/api/admin/account/save";
    if (checkSaveStatus()) {
...
editAccountInfo = function (index, id, restatus) {
    var status = $("#status:checked").val();
    var url = "/api/admin/account/accountEdit";
...
updatePerson = function (id) {
    ...
    var url = "/api/admin/account/personInfo";
    var data = {
        'id': id
    }
    $.ajax({
...
editPersonInfo = function (index, id) {
    ...
    var nodes = treeObj.getSelectedNodes();
    var companyId = nodes[0].id;
    var data = getPersonInfo(id);
    var url = "/api/admin/account/personInfoEdit"
    $.ajax({
        url: url,
        type: "post",
        data: JSON.stringify(data),
...
updateAuthority = function (id) {
    ...
    var url = "/api/admin/account/authorityInfo";
    var data = {
        'id': id
    }
    $.ajax({
        ...
        success: function (data) {
            ...
            roleList = accountVO.roleInfoList;
            roleListInit(roleList);
            setCheckRoleListInit(accountVO.roleIdList)
            layer.open({
                type: 1,
                title: "权限修改",
                btn: ['确定', '取消'],
...
getAuthorityInfo = function (id) {
    ...
    if (roleIdList == null || roleIdList.length < 1) {
        return layer.alert("请先选则角色", {icon: 5});
    }
    var data = {
        'id': id,
        'roleIdList': roleIdList
...
editAuthorityInfo = function (index, id) {
    ...
    var url = "/api/admin/account/updateRole"
    $.ajax({
        url: url,
        type: "post",
        data: JSON.stringify(data),
...
deleteAccount = function (id) {
    var url = "/api/admin/account/deleted";
